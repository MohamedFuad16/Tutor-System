...
export class BKTEngine {
   ...
   */
  public calculatePosterior(concept: PersistentConcept, isCorrect: boolean): number {
    ...
    let p_obs = (p_learn * p_obs_given_learned) + ((1 - p_learn) * p_obs_given_unlearned);

    // Filtered probability (P of knowing given the observation)
    let p_learn_given_obs = (p_learn * p_obs_given_learned) / Math.max(0.0001, p_obs);
  ...
  public async updateConceptAttempt(conceptId: string, isCorrect: boolean, type: "recognition" | "generation" | "transfer") {
    ...
    let maxPLearn = 0.95;
    if (type === "recognition" && posterior > 0.70) maxPLearn = 0.70;

...
export class LearnerModel {
  ...
  public async getLearnerSnapshot(currentConceptId?: string, sessionData?: any) {
    ...
    if (currentConceptId) {
      ...
      if (zpd.zpd_zone.length > 0) {
        let total = 0;
        zpd.zpd_zone.forEach(c => total += scaffoldingEngine.calculateScaffoldLevel(c));
        scaffold_level = Math.round(total / zpd.zpd_zone.length);
      }
    }

    const bkt_weak_kcs = await db.concepts.filter(c => c.p_learn < 0.6 && c.attempt_history.length > 0).limit(5).toArray();
    
    const loadState = sessionData ? cognitiveLoadMonitor.determineLoadConfig(
  ...
  public async getTutorInstructions(snapshot: any): Promise<string> {
    let instructions = "### VIRTUAL STUDENT BRAIN (Cognitive Science-Grounded Learner Model) DIRECTIVES ###\n\n";

    if (snapshot.active_misconceptions && snapshot.active_misconceptions.length > 0) {
       instructions += "- [CRITICAL] MISCONCEPTION DETECTED: Use Socratic correction. DO NOT just point out they are wrong directly. Ask questions to let them discover the flaw.\n";
    ...
    if (snapshot.scaffold_level !== undefined) {
      ...
      switch (snapshot.scaffold_level) {
        case 5: instructions += "Provide FULL worked examples, step-by-step walkthrough.\n"; break;
        case 4: instructions += "Provide worked example + leave ONE blank step for the student to fill.\n"; break;
        case 3: instructions += "Provide a PARTIAL hint + Socratic question.\n"; break;
        case 2: instructions += "Provide Socratic question ONLY. No direct answers.\n"; break;
        case 1: instructions += "Say 'Try it — I'm here if you need me'. Minimum intervention.\n"; break;
        case 0: instructions += "Student works independently. Validate and observe.\n"; break;
      }
    }

    if (snapshot.cognitive_load && snapshot.cognitive_load.level === "overload") {

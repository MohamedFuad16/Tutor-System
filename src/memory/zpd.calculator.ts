...
export class ZPDCalculator {
  public async getZones() {
    ...
    // Categorize
    const independent_zone = allConcepts.filter((c) => c.p_learn >= 0.9);
    const zpd_zone_candidates = allConcepts.filter(
      (c) => c.p_learn >= 0.4 && c.p_learn < 0.9,
    );
    const not_yet_candidates = allConcepts.filter((c) => c.p_learn < 0.4);
    ...
    for (const c of zpd_zone_candidates) {
      const ready = await prerequisiteDAG.arePrerequisitesMet(c.id, 0.7);
      if (ready) {
        zpd_zone.push(c);
      } else {
        not_yet_zone.push(c);

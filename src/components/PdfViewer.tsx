...
import { ZoomIn, ZoomOut, ChevronLeft, ChevronRight, Maximize, Minimize, Highlighter, MessageSquare, Underline, Strikethrough, StickyNote } from 'lucide-react';
...
export function PdfViewer() {
  ...
  const [selectionTooltip, setSelectionTooltip] = useState<{ x: number, y: number, text: string, rawRects: DOMRect[] } | null>(null);
  const [draftNote, setDraftNote] = useState<{ x: number, y: number, color: string, rects: any[] } | null>(null);
  ...
  const handlePageRenderSuccess = () => {
    if (pdfTotalPages > 0 && !titleGenerated) {
      const activeCanvas = document.querySelector('.react-pdf__Page__canvas') as HTMLCanvasElement;
        ...
        activeCanvas.toBlob((blob) => {
          ...
          reader.onloadend = () => {
            fetch('/api/title', {
              ...
              if (data.title) {
                 const cleanTitle = data.title.replace(/[^a-zA-Z0-9 -]/g, '').trim();
  ...
  const addSelectionAnnotation = (type: Annotation['type'], color: string) => {
    ...
    const normalizedRects = selectionTooltip.rawRects
      ...
      .map(rect => {
        ...
        const y = Math.max(0, Math.min(1, (rect.top - pageRect.top - yOffset) / pageRect.height));
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (selectionTooltip && !(e.target as Element).closest('.selection-tooltip-container')) {
        setSelectionTooltip(null);
        window.getSelection()?.removeAllRanges();
      }
    };
  ...
  const availHeight = containerHeight ? Math.max(containerHeight - 80, 200) : 1123;
  ...
  return (
    <div className="w-full h-full flex flex-col relative bg-[#0A0A0B]" ref={containerRef}>
      <style>{`
        ...
        >
          ...
          <div className="relative z-10 flex flex-col items-center justify-center px-4 min-w-[80px]">
            <span className="text-[14px] font-mono font-bold text-white tracking-widest flex items-center">
              ...
              />
              <span className="text-zinc-500 font-medium px-1">/</span> {pdfTotalPages || '-'}
            </span>
          </div>
      ...
      <div className="flex-1 w-full h-full overflow-auto pt-8 pb-32 flex justify-center custom-scroll bg-[#0A0A0B] relative">
        <div className="absolute inset-0 z-0 pointer-events-none" style={{ backgroundImage: 'radial-gradient(circle at center, rgba(255,255,255,0.03) 1px, transparent 1px)', backgroundSize: '24px 24px' }} />
        <div 
          ...
          <Document
            ...
            loading={
              <div 
                ...
                style={{ width: widthVal || defaultWidth, height: (widthVal || defaultWidth) * 1.414 }}
              >
                <motion.div 
                  animate={{ x: ['-100%', '200%'] }}
            ...
            {selectionTooltip && (
              ...
              <div className="relative flex items-center gap-1 p-1 bg-[#121214]/95 backdrop-blur-xl rounded-xl shadow-[0_20px_40px_rgba(0,0,0,0.8)] overflow-hidden">
                ...
                <div className="flex gap-1 pr-1.5 border-r border-white/10 relative z-10">
                  <button 
                    onClick={() => addSelectionAnnotation('highlight', '#fde047')} // yellow
                  ...
                  <button 
                    onClick={() => addSelectionAnnotation('underline', '#3b82f6')} // blue
                  ...
                  <button 
                    onClick={() => addSelectionAnnotation('strikethrough', '#ef4444')} // red
                ...
                >
                  ...
                  <MessageSquare size={12} className="relative z-10 drop-shadow-md" /> <span className="relative z-10 drop-shadow-sm font-bold">Ask Tutor</span>
                </button>
              </div>
              </motion.div>
            )}
            ...
            >
              <div className="bg-[#121214] border border-[#2A2A30] rounded-xl shadow-[0_20px_40px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col">
                <div className="bg-[#1A1A1E] px-3 py-2 border-b border-[#2A2A30] flex items-center justify-between">
                  ...
                  <button onClick={() => setDraftNote(null)} className="text-zinc-500 hover:text-white transition-colors">

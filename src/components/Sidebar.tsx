...
import { BookOpen, Compass, Clock, Settings, FolderClosed, Plus } from 'lucide-react';
...
export function Sidebar() {
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-3 space-y-6" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        <div className="space-y-1">
          <div className="px-2 text-[11px] font-medium text-[#555555] uppercase tracking-wider mb-2">Explore</div>
        ...
        <div className="space-y-1">
          <div className="px-2 text-[11px] font-medium text-[#555555] uppercase tracking-wider mb-2">Notebooks</div>
          <SidebarItem icon={<FolderClosed size={15} />} label="Quantum Physics Concept" />
          <SidebarItem icon={<FolderClosed size={15} />} label="Design Systems 2024" />
          <SidebarItem icon={<FolderClosed size={15} />} label="React Architectural Patterns" />
        </div>
      </div>
...
function SidebarItem({ icon, label, isActive = false }: { icon: React.ReactNode, label: string, isActive?: boolean }) {
  return (
    <button className={`w-full flex items-center gap-3 px-2 py-1.5 rounded-lg text-sm font-medium transition-colors focus:outline-none ${isActive ? "bg-[var(--color-canvas-elevated)] text-[var(--color-text-primary)] border border-white/[0.05]" : "text-[var(--color-text-secondary)] hover:bg-[#ffffff05] hover:text-[var(--color-text-primary)] border border-transparent"}`}>
      <span className={isActive ? "text-[var(--color-text-primary)]" : "text-[var(--color-text-secondary)]"}>{icon}</span>

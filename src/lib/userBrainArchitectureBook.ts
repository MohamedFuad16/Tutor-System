...
| Beta Diagnostics | Capped local export, readiness gate summary, live brain-flow coverage for chat, voice, both foreground tool layers, retrieval, model, both evaluated mastery layers, background memory evidence, model-observation gates, provider-key drill prompts with expected rows, and background job dead-letter blocking. Each live signal shows compact request/source/PDF anchors when evidence exists. The tab also includes a clearly separated in-memory synthetic wiring rehearsal that checks shared chat/voice tool schemas without counting toward live readiness. |

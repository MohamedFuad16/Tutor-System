...
- Coherent provider-key proof now separates completed model rows from provider-ready evidence: typed chat must include a completed OpenRouter-backed row for the selected request, and live voice must include the server-side Deepgram \`Voice provider ready\` system-activity row. The local \`Mock voice provider ready\` row stays visible for debugging but does not satisfy provider-key proof.
...
| Beta Diagnostics | Capped local export, readiness gate summary, live brain-flow coverage for chat, voice, both foreground tool layers, retrieval, model, both evaluated mastery layers, background memory evidence, model-observation gates, provider-key drill prompts with expected rows, real provider-ready row proof, and background job dead-letter blocking. Each live signal shows compact request/source/PDF anchors when evidence exists. The tab also includes a clearly separated in-memory synthetic wiring rehearsal that checks shared chat/voice tool schemas without counting toward live readiness. |

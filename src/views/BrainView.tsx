...
export function BrainView() {
  ...
  const fallbackConcepts =
    useLiveQuery(
      () => db.concepts.orderBy("lastReviewedAt").reverse().limit(80).toArray(),
      [],
    ) || [];
  ...
  const bookConceptsByBook = learningBookConcepts.reduce<
  ...
  const totalLearningConcepts =
    learningBookConcepts.length || fallbackConcepts.length;
    ...
    ? {
        nodes: [
          ...
          ...learningBooks.map((book) => ({
            ...
            val: Math.max(
              0.35,
              Math.min(1, (bookConceptsByBook[book.id]?.length || 1) / 8),
            ),
        ...
        links: [
          ...
          ...learningBookConcepts.flatMap((concept) =>
            (concept.childConcepts || [])
              .map((childName) =>
                learningBookConcepts.find(
                  (other) =>
                    other.bookId === concept.bookId &&
                    other.name.toLowerCase() === childName.toLowerCase(),
                ),
              )
              .filter(Boolean)
              .map((child) => ({
  ...
  return (
    <div
      ref={containerRef}
      className="w-full h-full bg-[#030303] relative overflow-hidden font-['Space_Grotesk']"
    >
        ...
        >
          ...
          <div className="relative ml-0 md:ml-28">
            ...
            <p className="mt-4 max-w-[58ch] text-[15px] md:text-[17px] font-light leading-relaxed tracking-tight text-zinc-300">
              {latestBook?.overview ||
                latestBook?.knowledgeSummary ||
                "A living map of the learner, their current session book, and the concepts the tutor is extracting after each conversation."}
            </p>
      ...
      <div className="absolute right-8 bottom-8 z-10 pointer-events-auto">
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 backdrop-blur-md rounded-full border border-white/10 text-white text-xs font-medium transition-[color,background-color,border-color,box-shadow,transform,opacity] focus:outline-none shadow-[0_4px_12px_rgba(0,0,0,0.5)]"
        >
      ...
      {dimensions.width > 0 && dimensions.height > 0 && (
        <ForceGraph3D
          ...
          nodeThreeObject={(node: any) => {
                ...
                : nodeType === "book"
                  ...
                  : rawVal > 0.7
                    ...
                    : rawVal > 0.4
                      ...
                      : "#f59e0b";
            const scaleBoost =
              nodeType === "learner" ? 1.45 : nodeType === "book" ? 1.18 : 1;

            const group = new THREE.Group();

            // 1. Cyber Core
            const coreGeom = getGeometry(
              `core_${nodeType}_${val.toFixed(2)}`,
              () =>
                new THREE.IcosahedronGeometry(
                  Math.sqrt(val) * 4 * scaleBoost,
                  0,
                ),
            );
            ...
            const prand = () => {
              hash = (Math.imul(31, hash) + 12345) | 0;
              return (hash >>> 0) / 4294967296;
            };

            // 2. Wireframe shell
            const shellGeom = getGeometry(
              `shell_${nodeType}_${val.toFixed(2)}`,
              () =>
                new THREE.IcosahedronGeometry(
                  Math.sqrt(val) * 7.5 * scaleBoost,
                  1,
                ),
            );
            const shellMat = getMaterial(
              `shell_${color}`,
              () =>
            ...
            const ringGeom = getGeometry(
              `ring_${nodeType}_${val.toFixed(2)}`,
              () =>
                new THREE.TorusGeometry(
                  Math.sqrt(val) * 11 * scaleBoost,
                  0.2,
                  16,
                  64,
                ),
            ...
            const glowGeom = getGeometry(
              `glow_${nodeType}_${val.toFixed(2)}`,
              () =>
                new THREE.SphereGeometry(
                  Math.sqrt(val) * 14 * glowMultiplier * scaleBoost,
                  32,
                  32,
                ),
            );
            const glowMat = getMaterial(
              `glow_${color}_${glowIntensity}`,
              () =>
                new THREE.MeshBasicMaterial({
                  color,
                  transparent: true,
            ...
            textSprite.position.set(
              0,
              Math.sqrt(val) * 11 * scaleBoost + 10,
              0,
            );

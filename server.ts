...
import { detectFreshnessSearch, formatSourcesForPrompt, searchSerper, type NormalizedWebSource, type WebSearchMode } from "./server/web-search";
...
type OpenRouterPricing = Record<string, { prompt: number; completion: number; name?: string }>;
...
const roundCost = (value: number) => Math.round((value || 0) * 1_000_000) / 1_000_000;
...
const voiceAgentCostForSeconds = (seconds: number) => roundCost((Math.max(0, seconds) / 60) * DEEPGRAM_PRICING.voiceAgentPerMinute);
...
const openRouterCost = (pricing: OpenRouterPricing, model: string, inputTokens: number, outputTokens: number) => {
...
const fallbackLearningUpdate = (body: any) => {
  ...
  const title = project && project !== "General Study"
    ? project
    : (userMessage.match(/\b(Python|JavaScript|React|TypeScript|Algorithms|System Design|Concurrency|Networking|Machine Learning|Calculus|History)\b/i)?.[0] || "General Study");
...
async function startServer() {
  ...
  console.log = (...args) => { broadcastLog("log", args); originalLog.apply(console, args); };
  console.error = (...args) => { broadcastLog("error", args); originalError.apply(console, args); };
  console.warn = (...args) => { broadcastLog("warn", args); originalWarn.apply(console, args); };
    ...
    try {
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are an expert prompt engineer. The user will give you a brief description of a persona for an AI Tutor. Write a highly detailed, professional System Prompt (in the first person or direct instruction) that the AI should follow to embody this persona. The output MUST ONLY be the raw system prompt text, nothing else. No prefixes like 'Here is the prompt'."
          },
          {
            role: "user",
            content: description
  ...
  app.post("/api/learning-book-update", async (req, res) => {
    ...
    const assistantMessage = String(body.assistantMessage || "").slice(0, 12000);
...
CRITICAL RULE: You MUST dynamically generate a specific, highly relevant \`chapterTitle\` based strictly on what the user actually asked about in this message (e.g. "List Comprehensions", "Promises and Async/Await", "Calculus Integrals"). DO NOT use generic chapter titles like "Conversation Notes". If the current book already exists, prefer continuing it and adding/refining chapters. Do not invent advanced concepts absent from the conversation.`
  ...
  app.post("/api/chat", async (req, res) => {
    ...
    try {
      ...
      const runWebSearch = async (query: string, mode: WebSearchMode = "search", maxResults = 6) => {
        ...
        try {
          const sources = await searchSerper({ query, mode, maxResults, apiKey: serperRuntimeKey || undefined });

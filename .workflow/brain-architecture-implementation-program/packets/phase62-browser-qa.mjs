...
async function openAdmin(metrics) {
  ...
  await waitFor(
    cdp,
    "document.body.innerText.toLowerCase().includes('coherent live proof bundle')",
  );
  return cdp;
}

async function checkAdminFallbackProofCopy(label, metrics) {
  ...
  const snapshot = await evaluate(
    ...
    `(() => {
      ...
      return {
        ...
        hasProviderKeyPanel: lower.includes("provider-key live proof"),
        hasCoherentPanel: lower.includes("coherent live proof bundle"),
        hasFallbackWarning: lower.includes("fallback model rows do not count for provider-key proof"),
        hasTypedChatBundle: lower.includes("typed chat request bundle"),
        hasVoiceBundle: lower.includes("live voice request bundle")
      };
    })()`,
  );
  ...
  if (missing.length) {
    throw new Error(
      `${label} missing fallback-proof evidence: ${missing.join(", ")}`,
    );
  }
  ...
  await fs.writeFile(
    `${outDir}/ABZ-admin-fallback-proof-${label}.png`,
...
const results = [
  await checkAdminFallbackProofCopy("desktop", {
    width: 1440,
    height: 1000,
    deviceScaleFactor: 1,
    mobile: false,
  }),
  await checkAdminFallbackProofCopy("mobile", {
    width: 390,
    height: 844,
    deviceScaleFactor: 2,
    mobile: true,
...
await fs.writeFile(
  `${outDir}/phase62-browser-qa.json`,

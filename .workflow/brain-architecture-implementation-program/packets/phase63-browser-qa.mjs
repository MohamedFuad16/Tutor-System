...
async function openAdmin(metrics) {
  ...
  await waitFor(
    cdp,
    "document.body.innerText.toLowerCase().includes('selected request rows')",
  );
  return cdp;
}

async function checkAdminBundleRows(label, metrics) {
  ...
  const snapshot = await evaluate(
    ...
    `(() => {
      ...
      return {
        ...
        selectedRowsCount: (lower.match(/selected request rows/g) || []).length,
        hasCoherentPanel: lower.includes("coherent live proof bundle"),
        hasTypedChatBundle: lower.includes("typed chat bundle"),
        hasLiveVoiceBundle: lower.includes("live voice bundle"),
        hasCompletedModelMetric: lower.includes("completed model"),
        hasTranscriptMetric: lower.includes("transcript"),
        hasBackgroundMetric: lower.includes("background"),
        hasNoRequestCopy: lower.includes("no request selected yet"),
        hasMissingRowsCopy: lower.includes("missing rows:")
      };
    })()`,
  );
  ...
  if (snapshot.selectedRowsCount < 2) {
    missing.push("selectedRowsCount>=2");
  }
  if (missing.length) {
    throw new Error(
      `${label} missing selected-bundle evidence: ${missing.join(", ")}`,
    );
  }
  ...
  await fs.writeFile(
    `${outDir}/ACA-admin-bundle-rows-${label}.png`,
...
const results = [
  await checkAdminBundleRows("desktop", {
    width: 1440,
    height: 1000,
    deviceScaleFactor: 1,
    mobile: false,
  }),
  await checkAdminBundleRows("mobile", {
    width: 390,
    height: 844,
    deviceScaleFactor: 2,
    mobile: true,
...
await fs.writeFile(
  `${outDir}/phase63-browser-qa.json`,

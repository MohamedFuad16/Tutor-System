...
async function openAdmin(metrics) {
  ...
  await waitFor(cdp, "document.body.innerText.toLowerCase().includes('live beta runbook')");
  return cdp;
}

async function checkAdminRunbook(label, metrics) {
  ...
  const snapshot = await evaluate(
    ...
    `(() => {
      ...
      return {
        ...
        hasActiveBookStep: lower.includes("use one multi-pdf active book"),
        hasTypedChatStep: lower.includes("run the typed-chat proof turn"),
        hasVoiceStep: lower.includes("run the live-voice proof turn"),
        hasBackgroundGate: lower.includes("wait for memory workers and gates"),
        hasCoherentExport: lower.includes("confirm coherent bundle and export"),
  ...
  if (missing.length) {
    throw new Error(`${label} missing runbook evidence: ${missing.join(", ")}`);
  }
  ...
  await fs.writeFile(
    `${outDir}/ABX-admin-runbook-${label}.png`,
...
const results = [
  await checkAdminRunbook("desktop", {
    width: 1440,
    height: 1000,
    deviceScaleFactor: 1,
    mobile: false,
  }),
  await checkAdminRunbook("mobile", {
    width: 390,
    height: 844,
    deviceScaleFactor: 2,
    mobile: true,
...
await fs.writeFile(
  `${outDir}/phase60-browser-qa.json`,

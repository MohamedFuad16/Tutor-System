...
async function openRevision(metrics) {
...
async function appDesignLatchCheck(label, metrics) {
  const cdp = await openRevision(metrics);
  ...
  await waitFor(
    cdp,
    "document.body.innerText.includes('Local Beta Control Patterns')",
  );
  await evaluate(cdp, scrollTextIntoView("proof-attempt id is latched"));
  await sleep(350);
  ...
  const snapshot = await evaluate(
    ...
    `(() => {
      ...
      return {
        ...
        hasControlPatterns: lower.includes("local beta control patterns"),
        hasVoiceTimeline: lower.includes("voice agent timeline"),
        hasLatchedProofCopy: lower.includes("proof-attempt id is latched at session start"),
        hasAttemptStabilityCopy: lower.includes("admin selection changes later")
      };
    })()`,
  );
  ...
  if (
    ...
    !snapshot.hasVoiceTimeline ||
    !snapshot.hasLatchedProofCopy ||
    !snapshot.hasAttemptStabilityCopy
  ) {
    throw new Error(
      `${label} App Design latch mismatch: ${JSON.stringify(snapshot)}`,
    );
  }
  assertNoOverflow(label, snapshot);
  await screenshot(cdp, `ACI-app-design-voice-latch-${label}.png`);
...
for (const viewport of viewports) {
  results.push(await appDesignLatchCheck(viewport.label, viewport.metrics));
}
...
await fs.writeFile(
  `${outDir}/phase70-browser-qa.json`,

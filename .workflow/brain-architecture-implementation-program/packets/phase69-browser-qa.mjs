...
async function adminDrillCheck(label, metrics) {
  ...
  await evaluate(cdp, scrollTextIntoView("Live proof drill packet"));
  await sleep(400);
  const snapshot = await evaluate(
    ...
    `(() => {
      ...
      return {
        ...
        hasDrillPacket: lower.includes("live proof drill packet"),
        hasPromptHeading: lower.includes("exact local prompts for chat and voice"),
        hasChatPrompt: lower.includes("typed-chat proof prompt") && lower.includes("provider-key proof turn") && lower.includes("all ready pdfs"),
        hasVoiceScript: lower.includes("live-voice proof script") && lower.includes("provider-key voice proof turn") && lower.includes("same active book"),
        hasExpectedRows: lower.includes("chat foreground tool job") && lower.includes("chat evaluated mastery evidence") && lower.includes("voice-agent tool job") && lower.includes("voice book_chat_thread_saved"),
        hasManualBoundary: lower.includes("cloud/aws deployment work") && lower.includes("local only"),
        hasExportInstructions: lower.includes("export instructions") && lower.includes("cloud sync artifact")
      };
    })()`,
  );
  if (
    !snapshot.hasDrillPacket ||
    !snapshot.hasPromptHeading ||
    !snapshot.hasChatPrompt ||
    !snapshot.hasVoiceScript ||
    !snapshot.hasExpectedRows ||
    !snapshot.hasManualBoundary ||
    !snapshot.hasExportInstructions
  ) {
    throw new Error(`${label} Admin drill mismatch: ${JSON.stringify(snapshot)}`);
  }
  assertNoOverflow(label, snapshot);
  await screenshot(cdp, `ACG-admin-drill-packet-${label}.png`);
  assertCleanConsole(label, cdp.consoleLogs);
  cdp.close();
  return { ...snapshot, consoleLogs: [] };
}

async function appDesignCheck(label, metrics) {
  ...
  await sleep(1000);
  await evaluate(cdp, scrollTextIntoView("Provider-key drill packet"));
  await sleep(300);
  ...
  const snapshot = await evaluate(
    ...
    `(() => {
      ...
      return {
        ...
        width: innerWidth,
        scrollWidth: document.documentElement.scrollWidth,
        hasAppDesign: lower.includes("app design language"),
        hasControlPatterns: lower.includes("local beta control surfaces"),
        hasDrillPattern: lower.includes("provider-key drill packet"),
        hasPromptCopy: lower.includes("exact typed-chat and live-voice prompts"),
        hasManualBoundary: lower.includes("without auto-starting provider calls or cloud work")
      };
    })()`,
  );
  if (
    !snapshot.hasAppDesign ||
    !snapshot.hasControlPatterns ||
    !snapshot.hasDrillPattern ||
    !snapshot.hasPromptCopy ||
    !snapshot.hasManualBoundary
  ) {
    throw new Error(
      `${label} App Design drill mismatch: ${JSON.stringify(snapshot)} body=${bodyText.slice(0, 1200)}`,
    );
  }
  assertNoOverflow(label, snapshot);
  await screenshot(cdp, `ACG-app-design-drill-packet-${label}.png`);
  assertCleanConsole(label, cdp.consoleLogs);
  cdp.close();
  return { ...snapshot, consoleLogs: [] };
}
...
for (const viewport of viewports) {
  results.push(await adminDrillCheck(viewport.label, viewport.metrics));
  results.push(await appDesignCheck(viewport.label, viewport.metrics));
}
...
await fs.writeFile(
  `${outDir}/phase69-browser-qa.json`,

...
const BOOK_ID = "acr-voice-script-book";
const THREAD_ID = "acr-voice-script-thread";
const DOCUMENT_IDS = ["acr-doc-active", "acr-doc-companion"];
    ...
    learningBooks: [
      {
        ...
        overview: "Local voice script handoff QA book.",
        summary: "Local voice script handoff QA book.",
        knowledgeSummary: "Local voice script handoff QA book.",
        chapters: [],
        conceptIds: [],
        conversationCount: 1,
        createdAt: BASE_TS,
    ...
    bookChatThreads: [
      {
        id: THREAD_ID,
        bookId: BOOK_ID,
        bookTitle: "ACR Voice Script Book",
        title: "ACR voice script thread",
        messages: [],
        createdAt: BASE_TS,
        updatedAt: BASE_TS + 20,
      },
...
async function runHandoffCheck(browser, label, viewport) {
  ...
  await page
    .locator("button", { hasText: "Load voice script" })
    .first()
    .click();
  await waitForText("Live proof capture");
  await waitForText("Voice script loaded");
  await page.waitForFunction(() =>
    Boolean(
      document
        .querySelector("textarea")
        ?.value.includes("Provider-key voice proof turn"),
    ),
  );
  await sleep(400);
  ...
  const snapshot = await page.evaluate((proofAttemptId) => {
    ...
    return {
      ...
      hasBook: lower.includes("acr voice script book"),
      hasVoiceScriptLoaded: lower.includes("voice script loaded"),
      hasStartVoiceFirst: lower.includes("start voice first"),
      ...
      textareaHasPrompt: Boolean(
        textarea?.value.includes("Provider-key voice proof turn"),
      ),
  ...
  if (
    ...
    !snapshot.hasAttempt ||
    !snapshot.hasBook ||
    !snapshot.hasVoiceScriptLoaded ||
    !snapshot.hasStartVoiceFirst ||
  ...
  await page.screenshot({
    path: `${outDir}/ACR-live-voice-script-handoff-${label}.png`,
    fullPage: false,
  });
...
try {
  ...
  await fs.writeFile(
    `${outDir}/phase75-live-voice-script-handoff-qa.json`,

...
function packScore(pack: Record<string, unknown>, tokens: string[]) {
  ...
  return Math.min(1, tokens.reduce((sum, token) => sum + (text.includes(token) ? 0.25 : 0), 0));
}
...
async function main() {
  ...
  const namedFiles = new Set(index.chunks.filter((chunk) => tokens.some((token) => chunk.file.toLowerCase().includes(token))).map((chunk) => chunk.file));
  ...
  const chunks = [...selected.values()].sort((a, b) => b.total - a.total).slice(0, 12).map((item) => ({
    file: item.chunk.file,
    startLine: item.chunk.startLine,
    endLine: item.chunk.endLine,
    kind: item.chunk.kind,
    score: Number(item.total.toFixed(4)),
    confidence: confidence(item.total),
    reasons: {
  ...
  console.log(JSON.stringify({
    ...
    chunks,
    retrievalConfidence: confidence(chunks[0]?.score ?? 0),
    tokenEfficiency: {

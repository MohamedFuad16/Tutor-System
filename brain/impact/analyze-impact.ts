...
function main() {
  ...
  if (!target)
    throw new Error("Usage: npm run brain:impact -- <file-or-symbol>");
  const impact = readJson<{
    metadata: Record<string, unknown>;
    files: Record<string, unknown>;
    couplingScores: Record<string, unknown>;
  ...
  const matches = Object.keys(impact.files).filter((file) =>
    file.includes(target),
  );
  const nodeMatches = graph.nodes.filter(
    (node) =>
      node.id.includes(target) ||
      node.label.includes(target) ||
      node.file?.includes(target),
  );
  ...
  const runtimeEvents = (runtime?.events ?? []).filter((event) => {
    ...
    return (
      [...files].some((file) =>
        haystack.includes(file.toLowerCase().replace(/^src\//, "")),
      ) ||
      haystack.includes(target.toLowerCase()) ||
      runtimeText.includes(String(event.id ?? "").toLowerCase())
    );
  });
  console.log(
    JSON.stringify(

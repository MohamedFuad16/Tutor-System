...
function normalizeTask(raw: Partial<TaskRecord>, index: number): TaskRecord {
  return {
    ...
    verificationResults:
      ...
      typeof raw.verificationResults === "object" &&
      !Array.isArray(raw.verificationResults)
        ? raw.verificationResults
        : {},
    unresolvedIssues: normalizeList(raw.unresolvedIssues),
    regenerationStatus:
      typeof raw.regenerationStatus === "string" && raw.regenerationStatus
        ? raw.regenerationStatus
        : "unknown",
...
function readMemory(): TaskRecord[] {
  ...
  return data.map((record, index) =>
    normalizeTask(record as Partial<TaskRecord>, index),
  );
}
...
function append(args: string[]) {
  const record: TaskRecord = {
    ...
    verificationResults: option(args, "verification")
      ? JSON.parse(option(args, "verification"))
      : args[5]
        ? JSON.parse(args[5])
        : {},
...
function linkTasks(records: TaskRecord[]) {
  return records.map((record) => {
    const linked = records
      ...
      .filter((candidate) => {
        const sharedFiles = candidate.filesChanged.some((file) =>
          record.filesChanged.includes(file),
        );
        const sharedInvariants = candidate.invariantsTouched.some((invariant) =>
          record.invariantsTouched.includes(invariant),
        );
        const sharedSystems = candidate.systemsAffected.some((system) =>
          record.systemsAffected.includes(system),
        );
...
function buildIndexes(records: TaskRecord[]): MemoryIndexes {
  ...
  linked.forEach((record) => {
    [...(record.decisions ?? []), ...record.invariantsTouched].forEach((key) =>
      addMapEntry(decisions, key, record),
    );
    record.risksIntroduced.forEach((risk) => addMapEntry(risks, risk, record));
    record.failureModes?.forEach((failure) =>
      addMapEntry(risks, `failure:${failure}`, record),
    );
    record.systemsAffected.forEach((system) =>
      addMapEntry(systems, system, record),
    );
    record.filesChanged.forEach((file) => addMapEntry(systems, file, record));
  });

  return {
    decisionChains: [...decisions.entries()]
    ...
    fragileSystems: [...systems.entries()]
      .map(([system, tasks]) => {
        const files = [
          ...new Set(tasks.flatMap((task) => task.filesChanged)),
        ].sort();
        const riskCount = tasks.reduce(
          (sum, task) => sum + task.risksIntroduced.length,
          0,
        );
        const failureCount = tasks.reduce(
          (sum, task) => sum + (task.failureModes?.length ?? 0),
          0,
        );
        const unresolvedCount = tasks.reduce(
          (sum, task) => sum + task.unresolvedIssues.length,
          0,
        );
        return {
          system,
...
function query(args: string[]) {
  ...
  const matches = records.filter((record) => {
    ...
    if (key)
      return normalizeList(record[key]).some((item) =>
...
function validate() {
  ...
  const result = {
    ...
    invalidTaskIds: invalid.map((record) => record.id),
  };

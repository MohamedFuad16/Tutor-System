...
function include(file: string) {
  return (
    ALLOW.some((pattern) => pattern.test(file)) &&
...
function currentFiles() {
  ...
  return Object.fromEntries(
    files
      ...
      .map((file) => [
        file,
        { hash: sha256(fs.readFileSync(path.join(ROOT, file))) },
      ]),
  );
}
...
function main() {
      ...
      JSON.stringify(
        {
          ok: false,
          stale: true,
          reason: "Missing snapshot. Run npm run brain:generate.",
        },
        null,
        2,
      ),
  ...
  const changed = Object.entries(current)
    .filter(([file, data]) => snapshot.files[file]?.hash !== data.hash)
    .map(([file]) => file);

...
type Graph = {
  ...
  edges: Array<{ id: string; type: string; source: string; target: string; confidence: number }>;
};
...
function tokenize(text: string) {
  const stop = new Set(["the", "and", "for", "with", "from", "this", "that", "into", "must", "should", "when", "where", "what"]);
...
async function embedQuery(text: string, sourceHash: string) {
  const cache = fs.existsSync(CACHE_PATH) ? JSON.parse(fs.readFileSync(CACHE_PATH, "utf8")) as Record<string, number[]> : {};
  const key = sha256(`${MODEL}:${sourceHash}:${text}`);
  if (cache[key]) return cache[key];
  const { pipeline } = await import("@xenova/transformers");
  const embedder = await pipeline("feature-extraction", MODEL);
  const output = await embedder(text.replace(/\s+/g, " ").trim().slice(0, 4000) || "empty", { pooling: "mean", normalize: true });
  const vector = Array.from(output.data as Float32Array).map((value) => Number(value.toFixed(6)));
  cache[key] = vector;
...
function loadContextPacks() {
  ...
  return fs.readdirSync(packsDir)
    ...
    .map((file) => JSON.parse(fs.readFileSync(path.join(packsDir, file), "utf8")) as Record<string, unknown>);
}

function packScore(pack: Record<string, unknown>, tokens: string[]) {
  const text = [pack.id, pack.title, ...(pack.invariants as string[] | undefined ?? []), ...(pack.sourceFiles as string[] | undefined ?? [])].join(" ").toLowerCase();
  const packTokens = new Set(tokenize(text));
  return Math.min(1, tokens.reduce((sum, token) => sum + (packTokens.has(token) ? 0.25 : 0), 0));
}

function taskMemoryScore(chunk: VectorChunk, tokens: string[]) {
  if (!chunk.file.includes("task-memory")) return 0;
  const text = chunk.text.toLowerCase();
  return Math.min(1, tokens.reduce((sum, token) => sum + (text.includes(token) ? 0.18 : 0), 0));
}

function graphNeighborScore(graph: Graph, chunk: VectorChunk, seedFiles: Set<string>) {
...
function retrievalConfidenceScore(items: Array<ScoredChunk & { confidenceScore: number }>) {
  if (!items.length) return 0;
  const topScore = items[0].confidenceScore;
  const topThree = items.slice(0, 3);
  const topThreeAverage = topThree.reduce((sum, item) => sum + item.confidenceScore, 0) / topThree.length;
  return clamp(Math.max(topScore, topThreeAverage));
}

async function main() {
  const task = process.argv.slice(2).join(" ");
  if (!task.trim()) throw new Error("Usage: npm run brain:retrieve-semantic -- <task>");
  const index = readJson<{ metadata: Record<string, unknown>; chunks: VectorChunk[] }>("brain/retrieval/vector-index.json");
  const graph = readJson<Graph>("brain/knowledge/graph.json");
  const tokens = tokenize(task);
  const querySparse = sparseVector(tokens);
  const queryEmbedding = await embedQuery(task, String(index.metadata.sourceHash ?? ""));
  const packs = loadContextPacks().map((pack) => ({ pack, score: packScore(pack, tokens) })).filter((item) => item.score > 0).sort((a, b) => b.score - a.score);
  const packFiles = new Set(packs.flatMap((item) => item.pack.sourceFiles as string[] | undefined ?? []));
  const namedFiles = new Set(index.chunks.filter((chunk) => {
    const pathText = searchablePath(chunk.file);
    return tokens.some((token) => hasToken(pathText, token) || (token.length >= 5 && chunk.file.toLowerCase().includes(token)));
  ...
  const scored = index.chunks.map((chunk) => {
    ...
    const total = (semantic * 0.55) + (lexical * 0.15) + (graphScore * 0.15) + (pack * 0.10) + (memory * 0.05);
  ...
  const calibrated = [...selected.values()].sort((a, b) => b.total - a.total).slice(0, 12).map((item) => ({
    ...item,
  ...
  console.log(JSON.stringify({
    ...
    retrievalFormula: "semantic .55 + lexical .15 + graph .15 + contextPack .10 + taskMemory .05",
    contextPacks: packs.slice(0, 6).map((item) => ({ id: item.pack.id, title: item.pack.title, score: item.score, sourceFiles: item.pack.sourceFiles, invariants: item.pack.invariants })),
    dependencyAwareFiles: [...new Set(chunks.map((chunk) => chunk.file))],
    chunks,
    retrievalConfidenceScore: Number(retrievalConfidenceScore(calibrated).toFixed(4)),
    ...
    tokenEfficiency: {
      ...
      estimatedSnippetLines: chunks.reduce((sum, chunk) => sum + Math.min(36, chunk.endLine - chunk.startLine + 1), 0),
    },
  }, null, 2));
}
